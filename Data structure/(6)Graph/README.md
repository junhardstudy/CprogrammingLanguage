...
```
주어진 그래프 구조에 대해, BFS로 탐색하는 함수입니다.

큐 구조는 앞에서 정의하고 여러번 사용했기 때문에, 여기서는 따로 설명해 놓지 않았습니다.

먼저 그래프 헤더(GRAPH* g)가 가리키는 첫번째 vertex를 큐에 넣습니다(ENQ함수). 

다음으로 큐에서 꺼내지고 나서(DEQ함수) 출력이 된 다음, 연결되어 있는(arc가 존재하는) vertex들을 큐에 넣게 됩니다.

이러한 과정이 큐의 사이즈가 0이 될때까지 반복됩니다.
...
```
주어진 그래프에 대해서, DFS로 탐색하는 함수입니다.

BFS에서와 마찬가지로, 스택을 이용한다는 점만 다르고 전체 흐름은 비슷합니다. 단 스택이기 때문에, 인접한 vertex들은 스택의 밑에 점점 쌓이게 되고
깊이가 우선시 되어 참조된 vertex들은 스택의 top또는 top근처에 위치하기 때문에 인접한 vertex보다 깊이를 우선한 vertex들이 먼저 탐색됩니다.

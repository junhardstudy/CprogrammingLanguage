...
class DynamicBindingTest{//동적바인딩 테스트
	public static void main(String [] args){
		...
		Ainmal [] animals = {new Animal(), new Human(), new Student()};
		animals[0].print();
		animals[1].Human();
		animals[2].Student();
		//컴파일 할 때는 해당 객체들을 묶고 있는 객체의 해당 메소드가 존재 하는지 유무를 판단
		//실제 실행 시에는 해당 객체의 메소드를 이용
		
		//메소드 호출 : 컴파일 여부 : 해당 객체의 자료형에 메소드가 있는지 없는지를 보고 컴파일 가부 결정, 실행 결과는 자료형이 아니라 실제 객체의 메소드 내용을 실행 이를 동적 바인딩이라 함}
